...
\newcommand{\rPedEqDef}
{
	\sset{((H,A_0,A_1),(a,r_0,r_1)))\colon \forall j \in \zo  \Ped_H(a;r_j) = A_j}
}

\newcommand{\PedGEqS}{{{\sf PedGrEq}}}
\newcommand{\rPedGEq}{\cR_\PedGEqS}
...
\begin{protocol}[Distributed key generation]\label{prot:TshEG:KG}~
	...
	\begin{enumerate}
		...
		\item 	\commentni{Generates the public key $E = e^{-1} \cdot H$}
		
		\Inote{The following sub-protocol  allows a corrupt party to induce wrong  $E$. This should be possible to  handled by verifying that the decryption works fine}
		\begin{enumerate}
		...
		\end{enumerate}
			...
			\item Output $(1^b,H,E \asn \sum_{\ell \in [n]} E_i')$ as the public key, $\set{E_\ell}_{\ell \in [n]}$ as the verification keys, and $e_i$ as the secret key.
