...
\begin{functionality}[\fConfTran: Confidential transactions]\label{func:ConfidentialTransactions}~
	...
	\begin{description}
		...
		\begin{enumerate}
			...
			\item \Assert{$d\in [n] \land x\in (\psize) \sland \tcount_d\le  \pcount$}.
			
			\item $\tcount_d^{++}$.
			
			\item $\pendbalance_d \pluseq x$.
			
			\item Set $\historylog \unioneq {(\mint,d,\tcount_d,x)}$.
		\end{enumerate}
		...
		\begin{enumerate}
			\item \Assert{$  d \in [n] \sland x\in (\psize) \sland \tcount\le  \pcount \sland \balance_s  \ge x $}.
		...
		\begin{enumerate}
			 ...
			 \item \Assert{$i \in [n] \sland  x\in (q) \sland \balance_i  \ge x$}.
...
\item[Operation:] ~
			...
			\begin{enumerate}
				...
				\begin{enumerate}
					...
					\item $\Enc_{\pk_i}(0),\oB_i\asn \Enc_{\pk_i}(0),\tcount_i 
					\asn 0$.
				\end{enumerate}
...
\item[Operation:] \Cc:
	...
	\item  $\historylog \pluseq (\rollover,i,\oB_i,\oP_i)$
