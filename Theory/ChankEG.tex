...
\Inote{Unfortunately, it seems we cannot use twisted EG here, since it has no simple  key-generation protocol  }
...
\begin{protocol}[Distributed key generation]\label{prot:TshEG:KG}~
	...
	\begin{enumerate}
		...
		\end{enumerate}		
			...
			\item Output $(1^b,H,?)$ as the public key, $\set{E_\ell}_{\ell \in [n]}$ as the verification keys, and $e_i$ as the secret key.
		\end{enumerate}
