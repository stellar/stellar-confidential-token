...
\begin{algorithm}[$(\EgKg,\EgEnc,\EgDec)$: Twisted ElGamal in-the-exponent encryption]\label{alg:tEGinExp}~
	...
	\begin{enumerate}
		...
		\item Find (using brute force) $m\in (-b, b) \in \Z_q$ so that $m\cdot G = M$. Abort if no such $m$ exists. \Inote{It seems that in our application it suffices to check for $m\in (b)$,}
...
\begin{enumerate}
	...
	\Inote{Sender refresh  his active balance }
