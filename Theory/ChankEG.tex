...
\newcommand{\rEgEqDef}
{
	\set{((E_0,E_1,\tA_0,\tA_1),(a,r_0,r_1))\colon \forall i\in \zo \colon \; \EgEnc_{E_i}(a;r_i)= \tA_i}
}
}
...
%\newcommand{\rEgRpDef}
%{
%	\set{((E,\tA,b),(a,r))\colon \EgEnc_E(a;r)= \tA \sland a\in (b)}
%}
...
\begin{enumerate}
	...
	\item Reconstruct $a$.
\end{enumerate}
In \cref{sec:ChanksEG:EG} we formally define the twisted ElGamal in-the-exponent scheme, and a few ZK proofs for the \NP-relations the scheme induces. The chunk ElGamal  scheme is defined in \cref{sec:ChanksEG:EGScheme} and the related ZK proofs, are defined in \cref{sec:ChanksEG:Proofs}. Finally, in \cref{sec:ChanksEG:Adjusting} we explain  how to adjust \cref{prot:ConfidentialTransactions} tp work with the new scheme.
...
\item[Key generation:] $\EgKG(1^b)$ samples $e\getsr \Z_q$, and outputs $(\sk \asn e,\pk \asn (1^b,E)\asn  e\cdot G)$.
	
	\item[Encryption:] $\EgEnc_E(a)$  samples $r\getsr \Z_q$, and outputs  $\tA \asn (r\cdot G, r\cdot E + a \cdot G)$
	
	
		\item[Decryption:] $\EgDec_{e,1^b}(\tA)$,   
		\begin{enumerate}
			\item 	Let $M \asn \tA_2 - e\cdot \tA_1$.
...
\end{algorithm}	
When clear from the context,  will omit the parameter $1^b$ from the public  key of the scheme.
...
\begin{description}
	...
	\item[Proof:] We use \textit{bullet proofs} \cite{SP:BBBPWM18}. \Inote{Are we using \textit{twisted ElGamal?}} 
\end{description}
...
Throughout we fix a cyclic  additive $q$-size group $\cG$ with generators $G$. The  twisted ElGamal in-the-exponent encryption scheme $(\tEgKG,\tEgEnc,\tEgDec)$ is defined below/ Note that it  gets  $H\in \cG$ as an  additional parameter.%\footnote{We will later assume that it is hard to compute the discrete log log $H$ in \wrt $G$, which holds assuming $H$ is sampled at random from the $\cG$.} 
...
\paragraph{Warning!} The  following proofs are sound only if the prover does \emph{not} known the discrete log of $H$ \wrt to $G$.\footnote{This holds, for instance,  assuming discrete  log is hard over $\cG$, and $H,G$ are chosen independently  and uniformly in $\cG$.}
